...
| --- | --- | --- | --- | --- |
| jar_cache_key | string | yes |  |  |
| release_tag | string | no | main |  |
| setup_script | string | yes |  |  |
| app_name | string | yes |  |  |
...
| Restore cached JAR | actions/cache/restore@v3 |  |
| Check JAR presence |  | `run` command |
| Get JAR filename |  | `run` command |
| Install Inno Setup |  | `run` command |
| Validate Inno Setup install |  | `run` command |
| Build setup.exe with Inno Setup |  | `run` command |
| Debug - Check what was created |  | `run` command |
| Set output installer path |  | `run` command |
| Upload installer artifact | actions/upload-artifact@v4 |  |
| Installer Build Complete |  | `run` command |
...
jobs:
  build-installer:
    ...
    steps:
      ...
      - name: Set output installer path
        ...
        run: |
          ...
          echo "installer_file=$($installer.FullName)" >> $env:GITHUB_OUTPUT
