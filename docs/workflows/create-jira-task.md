...
| name | action | run |
| --- | --- | --- |
| Checkout repository (full) | actions/checkout@v4 |  |
| Gather changed files |  | `run` command |
| Check for app file changes |  | `run` command |
| Create Jira issue when app files changed |  | `run` command |
| Upload created issue metadata | actions/upload-artifact@v4 |  |
