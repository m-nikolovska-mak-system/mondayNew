...
**Source:** `test-file-detection.yml`
...
| name | action | run |
| --- | --- | --- |
| Show what was detected |  | `run` command |
| Create test summary |  | `run` command |
    ...
    steps:
      - name: Show what was detected
        run: |
          ...
          if [ "${{ needs.test-detection.outputs.files_changed }}" = "true" ]; then
            echo "✅ STATUS: Watched files CHANGED"
            echo ""
            echo "📝 Changed watched files:"
            echo "${{ needs.test-detection.outputs.changed_files_list }}" | tr ',' '\n' | sed 's/^/  ✓ /'
          else
            echo "❌ STATUS: No watched files changed"
          fi
          
          echo ""
          echo "📂 All files changed in release:"
      ...
      - name: Create test summary
        run: |
          ...
          if [ "${{ needs.test-detection.outputs.files_changed }}" = "true" ]; then
            echo "## ✅ Result: Files Changed" >> $GITHUB_STEP_SUMMARY
            echo "" >> $GITHUB_STEP_SUMMARY
