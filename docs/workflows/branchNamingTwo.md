...
| Validate branch name on create |  | `run` command |
| Validate branch name on push | deepakputhraya/action-branch-name@master |  |
| Extract Jira Ticket Key |  | `run` command |
| Validate Jira Ticket Exists |  | `run` command |
| Notify Microsoft Teams on failure |  | `run` command |
| Create GitHub Issue on violation |  | `run` command |
| Comment on PR if branch name is invalid | marocchino/sticky-pull-request-comment@v2 |  |
  ...
  branch-naming-rules:
    ...
    steps:
      ...
      - name: Notify Microsoft Teams on failure
        ...
        run: |
          ...
          -d '
          {
            ...
            "text": "Branch: ${{ github.ref }}\nPlease use:\n- INT-feature-login-ERP-12345\n- hot-fix-INT-login-ERP-12345"
          }
          ' \
          ${{ secrets.TEAMS_WEBHOOK_URL }}

      - name: Create GitHub Issue on violation
        if: failure()
        run: |
          BRANCH_NAME="${GITHUB_REF#refs/heads/}"
          gh issue create \
      ...
      - name: Comment on PR if branch name is invalid
        if: failure() && github.event.pull_request.head.ref != ''
        uses: marocchino/sticky-pull-request-comment@v2
        with:
          message: |
            ⚠️ **Branch name does not follow the convention or Jira ticket is invalid!**
