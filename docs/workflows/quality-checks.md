# Simple Quality Checks

**Source:** `quality-checks.yml`

## Triggers
- `push`
- `pull_request`
- `workflow_dispatch`

## Inputs
_None_

## Outputs
_None_

## Secrets
_None_

## Jobs
### quality-checks

| name | action | run |
| --- | --- | --- |
| 📥 Checkout code | actions/checkout@v4 |  |
| ✅ Check README exists |  | `run` command |
| 📄 Check README has content |  | `run` command |
| 🐚 Check shell scripts |  | `run` command |
| 📝 Check Markdown files |  | `run` command |
| 📊 Summary Report |  | `run` command |

## Full YAML
```yaml
name: Simple Quality Checks

on:
  push:
  pull_request:
  workflow_dispatch:

jobs:
  quality-checks:
    runs-on: ubuntu-latest
    
    steps:
      - name: 📥 Checkout code
        uses: actions/checkout@v4
      
      - name: ✅ Check README exists
        id: readme_exists
        run: |
          echo "🔍 Checking if README.md exists..."
          if [ -f README.md ]; then
            echo "✅ README.md found"
            echo "status=pass" >> $GITHUB_OUTPUT
          else
            echo "❌ README.md not found"
            echo "status=fail" >> $GITHUB_OUTPUT
            exit 1
          fi
      
      - name: 📄 Check README has content
        id: readme_content
        if: always()
        run: |
          echo "🔍 Checking README.md has content..."
          if [ ! -f README.md ]; then
            echo "⚠️  Skipping (README doesn't exist)"
            echo "status=skip" >> $GITHUB_OUTPUT
            exit 0
          fi
          
          lines=$(wc -l < README.md)
          if [ "$lines" -gt 5 ]; then
            echo "✅ README has $lines lines (minimum 5)"
            echo "status=pass" >> $GITHUB_OUTPUT
          else
            echo "⚠️  README only has $lines lines (expected at least 5)"
            echo "status=warn" >> $GITHUB_OUTPUT
          fi
        continue-on-error: true
      
      - name: 🐚 Check shell scripts
        id: shell_check
        if: always()
        run: |
          echo "🔍 Looking for shell scripts..."
          shfiles=$(find . -name "*.sh" -type f 2>/dev/null | wc -l)
          
          if [ "$shfiles" -eq 0 ]; then
            echo "ℹ️  No shell scripts found - skipping"
            echo "status=skip" >> $GITHUB_OUTPUT
            exit 0
          fi
          
          echo "Found $shfiles shell script(s)"
          has_issues=0
          
          # Check for empty scripts
          empty=$(find . -name "*.sh" -type f -empty 2>/dev/null)
          if [ -n "$empty" ]; then
            echo "⚠️  Empty shell scripts found:"
            echo "$empty"
            has_issues=1
          else
            echo "✅ No empty shell scripts"
          fi
          
          # Check for shebangs
          for script in $(find . -name "*.sh" -type f 2>/dev/null); do
            if ! head -n1 "$script" | grep -q '^#!'; then
              echo "⚠️  Missing shebang: $script"
              has_issues=1
            fi
          done
          
          if [ "$has_issues" -eq 0 ]; then
            echo "✅ All shell scripts are valid"
            echo "status=pass" >> $GITHUB_OUTPUT
          else
            echo "status=warn" >> $GITHUB_OUTPUT
          fi
        continue-on-error: true
      
      - name: 📝 Check Markdown files
        id: markdown_check
        if: always()
        run: |
          echo "🔍 Installing markdownlint..."
          npm install -g markdownlint-cli >/dev/null 2>&1
          
          echo "🔍 Checking Markdown files..."
          if markdownlint '**/*.md' --ignore node_modules 2>&1; then
            echo "✅ All Markdown files are valid"
            echo "status=pass" >> $GITHUB_OUTPUT
          else
            echo ""
            echo "⚠️  Some Markdown issues found (see above)"
            echo "status=warn" >> $GITHUB_OUTPUT
          fi
        continue-on-error: true
      
      - name: 📊 Summary Report
        if: always()
        run: |
          echo ""
          echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
          echo "📊 QUALITY CHECKS SUMMARY"
          echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
          echo ""
          
          # README exists
          status="${{ steps.readme_exists.outputs.status }}"
          if [ "$status" = "pass" ]; then
            echo "✅ README exists: PASS"
          elif [ "$status" = "fail" ]; then
            echo "❌ README exists: FAIL"
          fi
          
          # README content
          status="${{ steps.readme_content.outputs.status }}"
          if [ "$status" = "pass" ]; then
            echo "✅ README content: PASS"
          elif [ "$status" = "warn" ]; then
            echo "⚠️  README content: WARNING"
          elif [ "$status" = "skip" ]; then
            echo "⏭️  README content: SKIPPED"
          fi
          
          # Shell scripts
          status="${{ steps.shell_check.outputs.status }}"
          if [ "$status" = "pass" ]; then
            echo "✅ Shell scripts: PASS"
          elif [ "$status" = "warn" ]; then
            echo "⚠️  Shell scripts: WARNING"
          elif [ "$status" = "skip" ]; then
            echo "⏭️  Shell scripts: SKIPPED"
          fi
          
          # Markdown
          status="${{ steps.markdown_check.outputs.status }}"
          if [ "$status" = "pass" ]; then
            echo "✅ Markdown files: PASS"
          elif [ "$status" = "warn" ]; then
            echo "⚠️  Markdown files: WARNING"
          fi
          
          echo ""
          echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
          
          # Check if README exists failed (critical)
          if [ "${{ steps.readme_exists.outputs.status }}" = "fail" ]; then
            echo ""
            echo "❌ FAILED: Critical check failed (README must exist)"
            exit 1
          fi
          
          echo ""
          echo "✨ All critical checks passed!"
          echo "⚠️  Warnings don't fail the workflow"
```