...
| --- | --- | --- | --- | --- |
| notification_title | string | yes |  | Title of the notification card |
| action_required_message | string | no | ⚠️ **Reminder:** Changes detected that may require action | Message to display in the attention container |
| card_color | string | no | Accent | Color accent for the card title (Accent, Good, Warning, Attention) |
| show_files | boolean | no | true | Whether to show changed files in the notification |
| filter_path_pattern | string | no |  | Optional regex pattern to filter changed files |
...
| Detect changed files |  | `run` command |
| Send Microsoft Teams notification |  | `run` command |

## Full YAML
```yaml
name: Teams Notification Template THIS IS THE GOOD ONE
  ...
  workflow_call:
    inputs:
      ...
      show_files:
        ...
        default: true
      filter_path_pattern:
        description: 'Optional regex pattern to filter changed files'
        required: false
        type: string
        default: ''
    secrets:
    ...
    steps:
      ...
      - name: Detect changed files
        ...
        run: |
          ...
          if [ -n "${{ inputs.filter_path_pattern }}" ]; then
            echo "🔍 Filtering changed files with pattern: ${{ inputs.filter_path_pattern }}"
            CHANGED=$(echo "$CHANGED" | grep -E "${{ inputs.filter_path_pattern }}" || true)
          fi
          fi

          if [ -z "$CHANGED" ]; then
            echo "⚠️  No files changed — skipping notification."
            echo "found=false" >> $GITHUB_OUTPUT
            exit 0
          fi

          echo "✅ Found $(echo "$CHANGED" | wc -l) changed file(s)"
