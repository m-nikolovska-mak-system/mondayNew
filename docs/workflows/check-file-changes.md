...
| --- | --- | --- | --- | --- |
| file_pattern | string | yes |  | Regex pattern for files to check |
| current_ref | string | no |  | Current ref (tag/branch) |
| previous_ref | string | no |  | Previous ref (auto-detected if empty) |

## Outputs
| name | description |
| --- | --- |
| changed | Whether files changed |
| files | List of changed files |
| current_ref | Current ref used |
...
| Resolve refs |  | `run` command |
| Detect changes |  | `run` command |
      ...
      - name: Detect changes
        ...
        run: |
          ...
          if [ -n "$changed_files" ]; then
            ...
            echo "ℹ️ No matching files changed"
          fi
