...
**Source:** `generate-readme-simple.yml`
...
| Select workflow file |  | `run` command |
| Ensure docs folder exists |  | `run` command |
| Prepare initial README if needed |  | `run` command |
| Run auto-doc | tj-actions/auto-doc@v3 |  |
| Commit README changes | stefanzweifel/git-auto-commit-action@v5 |  |
| Show result |  | `run` command |
...
jobs:
  generate-readme:
    ...
    steps:
      ...
      - name: Prepare initial README if needed
        run: |
          ...
          if [ ! -f "$FILE" ]; then
            ...
            echo "" >> "$FILE"
            echo "> Auto-generated section below:" >> "$FILE"
            echo "" >> "$FILE"
          fi
